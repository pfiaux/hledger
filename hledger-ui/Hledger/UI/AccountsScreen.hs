    ...
    regscr = rsSetAccount selacct isdepthclipped registerScreen
      where
        isdepthclipped = case getDepth ui of
                          ...
                          Nothing -> False
  -- try to avoid tricky state bugs
  -- XXX something is still wrong since the brick 1.0 migration.
  -- After putting this new ui state the current screen is regscr
  -- and brick should be calling rsHandle, but somehow it keeps
  -- calling asHandle.
  let ui' = screenEnter d regscr ui
  put' ui'
  rsCenterSelection ui'
  -- get >>= dlogUiTraceM . pshow
